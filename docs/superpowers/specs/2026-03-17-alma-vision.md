# VIVARA Core Platform — Spec Document (formerly ALMA)
...
| Insight | Growth | VIVARA가 사용자 데이터에서 발견한 성장 관련 인사이트 |
| AgentMaturity | Growth | VIVARA 자체의 학습 수준 (사용자 이해도, 자동화 정확도) |
...
VIVARA에서 **성장**은 세 가지 축으로 정의된다:

| 성장 축 | 주체 | 설명 |
|---------|------|------|
| **사용자의 성장** | 인간 | 목표 설정 → 진행 추적 → 회고 → 다음 단계 제시. VIVARA가 사용자의 삶의 방향을 이해하고 성장 여정을 함께한다 |
| **에이전트의 성장** | VIVARA | 대화할수록 사용자를 더 잘 이해하고, 더 정확한 자동화를 수행하며, 더 관련성 높은 지식을 제공한다 |
| **상호 성장** | 함께 | 사용자가 VIVARA를 가르치고, VIVARA가 사용자에게 인사이트를 제공하는 양방향 성장 루프 |
...
| `AgentMaturity` | VIVARA 자체 학습 수준 추적 | 엔티티 |
...
### 15.6 VIVARA 자체 성장 (Agent Maturity)
...
| L1 제안 | VIVARA가 제안, 사용자 승인 후 실행 | "내일 회의 전에 자료 정리할까요?" |
