...
# Test — requires DATABASE_URL + ENCRYPTION_KEY (Fernet key for at-rest encryption)
DATABASE_URL="postgresql+asyncpg://alma:alma@localhost:5433/alma_test" \
  TEST_DATABASE_URL="postgresql+asyncpg://alma:alma@localhost:5433/alma_test" \
  ENCRYPTION_KEY="<fernet-key>" pytest -v
...
### Docker (from root)
```bash
docker-compose up -d                      # Full stack (nginx:80 → backend:8000, frontend:3000)
docker-compose --profile test up db-test  # Test DB only (port 5433)
```

## Environment Variables

Required in `backend/.env` (see `config.py` for all keys):

| Variable | Required | Notes |
|----------|----------|-------|
| `DATABASE_URL` | Yes | `postgresql+asyncpg://...` |
| `JWT_SECRET` | Yes | Token signing key |
| `ENCRYPTION_KEY` | Yes | Fernet key — also needed for tests |
| `ANTHROPIC_API_KEY` | For Claude LLM | |
| `OPENAI_API_KEY` | For GPT + OpenAI embeddings | |
| `GEMINI_API_KEY` | For Gemini LLM + embeddings (primary) | |
| `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` | For Google Calendar OAuth | |
| `VAPID_PRIVATE_KEY` / `VAPID_PUBLIC_KEY` | For Web Push | |
| `TELEGRAM_BOT_TOKEN` / `DISCORD_BOT_TOKEN` | For gateway channels | |

Generate a Fernet key: `python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"`

## CI Pipeline

`push`/`PR` → **backend-lint** → **backend-test** (pgvector:pg16 service) → **frontend-lint** → **frontend-build** → **docker-build**

Tests run against a fresh pg16 container (not Supabase). CI sets `ENCRYPTION_KEY` explicitly.
...
- 18 migration files total (initial → vector → integrations → goals → habits → events → ontology)
